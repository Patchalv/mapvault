...
```json
{
  ...
  "locationBias": { "latitude": 40.4168, "longitude": -3.7038 },
  "languageCode": "en" | "es"
}
```
...
3. If premium:
   - Reject with `429 DISCOVER_DAILY_CAP` if today's usage ≥ 50 (compute from a derived `discover_search_events` table or a daily-window column on `profiles`).
...
**Flow:**
1. Validate auth.
2. For each photoName, `GET https://places.googleapis.com/v1/{photoName}/media?maxWidthPx=...&skipHttpRedirect=true` → parse `photoUri` from the JSON response.
...
| **Trial exhausted** | Edge Function returns `402` | Overlay shows paywall trigger (`router.push('/(tabs)/settings/paywall?trigger=discover_limit')`). |
| **Daily cap (premium)** | Edge Function returns `429` | Overlay shows `t('discover.dailyCap')` — soft cap. |
...
- On success: `react-native-toast-message` shows `t('discover.savedSuccess')`. Button transitions to "Already saved".
...
|-------|-----------|-------|
| `discover_search_submitted` | `query: string` (raw, truncated to 200), `result_count: number`, `from_cache: boolean` | Raw query collected for product insight. See compliance below. |
...
| `discover_cleared` | — | |
| `discover_trial_exhausted` | — | Fires on 402 from Edge Function. |
| `discover_paywall_shown` | `trigger: 'trial_exhausted' \| 'save_limit'` | |
| `discover_daily_cap_hit` | — | Premium-only. |
